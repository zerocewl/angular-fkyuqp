import { __decorate, __metadata, __values, __spread, __param, __awaiter, __generator, __assign, __extends } from 'tslib';
import { Injectable, ComponentFactoryResolver, ApplicationRef, Injector, Optional, Input, Component, Output, Inject, ChangeDetectorRef, ElementRef, EventEmitter, NgModule } from '@angular/core';
...
function getAssociatedDateSorter(fieldType) {
    var FORMAT = (fieldType === FieldType.date) ? moment$1.ISO_8601 : mapMomentDateFormatWithFieldType(fieldType);
...
var ExcelExportService = /** @class */ (function () {
    ...
    ExcelExportService.prototype.exportToExcel = function (options) {
        ...
        return new Promise(function (resolve, reject) {
            ...
            _this._workbook = new Workbook();
            _this._sheet = new Worksheet({ name: _this._excelExportOptions.sheetName || 'Sheet1' });
                ...
                return __generator(this, function (_a) {
                    switch (_a.label) {
                        case 0:
                            ...
                            return [4 /*yield*/, Builder.createFile(this._workbook, { type: 'blob' })];
                        case 1:
                            excelBlob = _a.sent();
                            downloadOptions = {
...
var CellMenuExtension = /** @class */ (function () {
    ...
    CellMenuExtension.prototype.register = function () {
        ...
        if (this.sharedService && this.sharedService.grid && this.sharedService.gridOptions) {
            ...
            this.sharedService.gridOptions.cellMenu = __assign({}, this.getDefaultCellMenuOptions(), this.sharedService.gridOptions.cellMenu);
...
var ContextMenuExtension = /** @class */ (function () {
    ...
    ContextMenuExtension.prototype.register = function () {
        ...
        if (this.sharedService && this.sharedService.grid && this.sharedService.gridOptions && this.sharedService.gridOptions.contextMenu) {
            ...
            this._userOriginalContextMenu = __assign({}, contextMenu_1);
            // dynamically import the SlickGrid plugin (addon) with RequireJS
            this.extensionUtility.loadExtensionDynamically(ExtensionName.contextMenu);
            this.sharedService.gridOptions.contextMenu = __assign({}, contextMenu_1);
            // sort all menu items by their position order when defined
...
var AutoCompleteFilter = /** @class */ (function () {
    ...
    AutoCompleteFilter.prototype.createDomElement = function (filterTemplate, collection, searchTerm) {
        ...
        if (autoCompleteOptions) {
            ...
            this._autoCompleteOptions = __assign({}, definedOptions, this.columnFilter.filterOptions);
            ...
            if (gridOptions && gridOptions.rowDetailView) {
                if (!this._addon) {
                    ...
                    if (!gridOptions.rowDetailView.postTemplate) {
                        ...
                        gridOptions.rowDetailView.postTemplate = function (itemDetail) { return DOMPurify$1.sanitize("<div class=\"" + ROW_DETAIL_CONTAINER_PREFIX + itemDetail[_this.datasetIdPropName] + "\"></div>"); };
                    }
...
var DateEditor = /** @class */ (function () {
    ...
    DateEditor.prototype.applyValue = function (item, state) {
        ...
        // validate the value before applying it (if not valid we'll set an empty string)
        var validation = this.validate(state);
        var newValue = (validation && validation.valid) ? moment$5(state, outputFormat).toDate() : '';
        // set the new value to the item datacontext
        if (isComplexObject) {
            setDeepValue(item, fieldName, newValue);
        }
    ...
    DateEditor.prototype.isValueChanged = function () {
        ...
        var outputFormat = mapMomentDateFormatWithFieldType(this.columnDef.type || FieldType.dateIso);
        var elmDateStr = elmValue ? moment$5(elmValue).format(outputFormat) : '';
        var orgDateStr = this.originalDate ? moment$5(this.originalDate).format(outputFormat) : '';
    ...
    DateEditor.prototype.serializeValue = function () {
        ...
        var outputFormat = mapMomentDateFormatWithFieldType(this.columnDef.type || FieldType.dateIso);
        var value = moment$5(domValue).format(outputFormat);
        return value;
    };
...
var LongTextEditor = /** @class */ (function () {
    ...
    LongTextEditor.prototype.init = function () {
        ...
        // so register it here
        if (this.hasAutoCommitEdit) {
            this._$textarea.on('focusout', function () { return _this.save(); });
        }
        ...
        if (collection.every(function (x) { return typeof x === 'string'; })) {
            ...
            collection.forEach(function (option) {
                ...
                if (isRenderHtmlEnabled) {
                    // sanitize any unauthorized html tags like script and others
                    // for the remaining allowed tags we'll permit all attributes
                    var sanitizedText = (DOMPurify$2.sanitize(optionText, sanitizedOptions) || '').toString();
                    optionText = htmlEncode(sanitizedText);
                }
...
function getAssociatedDateFormatter(fieldType, defaultSeparator) {
    ...
    return function (row, cell, value, columnDef, dataContext, grid) {
        ...
        var isDateValid = moment$6(value, defaultDateFormat, false).isValid();
        var outputDate = (value && isDateValid) ? moment$6(value).format(defaultDateFormat) : value;
...
var collectionFormatter = function (row, cell, value, columnDef, dataContext) {
    ...
    if (Array.isArray(value)) {
        return arrayToCsvFormatter(row, cell, value.map(function (v) { return findOrDefault(collection, function (c) { return c[valueName] === v; })[labelName]; }));
    }
...
var collectionEditorFormatter = function (row, cell, value, columnDef, dataContext) {
    ...
    if (Array.isArray(value)) {
        if (collection.every(function (x) { return typeof x === 'string'; })) {
            return arrayToCsvFormatter(row, cell, value.map(function (v) { return findOrDefault(collection, function (c) { return c === v; }); }));
        }
        else {
            return arrayToCsvFormatter(row, cell, value.map(function (v) { return findOrDefault(collection, function (c) { return c[valueName] === v; })[labelName]; }));
        }
...
export { Aggregators, AngularSlickgridComponent, AngularSlickgridModule, AngularUtilService, AutoTooltipExtension, BsDropDownService, CaseType, CellExternalCopyManagerExtension, CellMenuExtension, CheckboxSelectorExtension, CollectionService, ColumnPickerExtension, ContextMenuExtension, DelimiterType, DraggableGroupingExtension, Editors, EmitterType, ExcelExportService, ExportService, ExtensionName, ExtensionService, ExtensionUtility, FieldType, FileType, FilterConditions, FilterFactory, FilterMultiplePassType, FilterService, Filters, Formatters, GraphqlService, GridEventService, GridMenuExtension, GridOdataService, GridService, GridStateService, GridStateType, GroupItemMetaProviderExtension, GroupTotalFormatters, GroupingAndColspanService, HeaderButtonExtension, HeaderMenuExtension, KeyCode, OdataQueryBuilderService, OperatorType, PaginationService, ResizerService, RowDetailViewExtension, RowMoveManagerExtension, RowSelectionExtension, SharedService, SlickPaginationComponent, SlickgridConfig, SortDirection, SortDirectionNumber, SortService, Sorters, addToArrayWhenNotExists, addWhiteSpaces, castToPromise, charArraysEqual, decimalFormatted, executeBackendCallback, executeBackendProcessesCallback, findOrDefault, formatNumber, getDescendantProperty, getScrollBarWidth, htmlDecode, htmlEncode, htmlEntityDecode, htmlEntityEncode, mapFlatpickrDateFormatWithFieldType, mapMomentDateFormatWithFieldType, mapOperatorByFieldType, mapOperatorToShorthandDesignation, mapOperatorType, onBackendError, parseBoolean, parseUtcDate, refreshBackendDataset, sanitizeHtmlToText, setDeepValue, thousandSeparatorFormatted, titleCase, toCamelCase, toKebabCase, toSnakeCase, uniqueArray, uniqueObjectArray, unsubscribeAllObservables, AvgAggregator as ɵa, MinAggregator as ɵb, CompoundSliderFilter as ɵba, DateRangeFilter as ɵbb, InputFilter as ɵbc, InputMaskFilter as ɵbd, InputNumberFilter as ɵbe, InputPasswordFilter as ɵbf, MultipleSelectFilter as ɵbg, SelectFilter as ɵbh, NativeSelectFilter as ɵbi, SingleSelectFilter as ɵbj, SliderFilter as ɵbk, SliderRangeFilter as ɵbl, arrayObjectToCsvFormatter as ɵbm, arrayToCsvFormatter as ɵbn, boldFormatter as ɵbo, bsDropdownFormatter as ɵbp, checkboxFormatter as ɵbq, checkmarkFormatter as ɵbr, complexObjectFormatter as ɵbs, collectionFormatter as ɵbt, collectionEditorFormatter as ɵbu, getAssociatedDateFormatter as ɵbv, deleteIconFormatter as ɵbw, decimalFormatter as ɵbx, dollarFormatter as ɵby, dollarColoredFormatter as ɵbz, MaxAggregator as ɵc, dollarColoredBoldFormatter as ɵca, editIconFormatter as ɵcb, hyperlinkFormatter as ɵcc, iconFormatter as ɵcd, infoIconFormatter as ɵce, italicFormatter as ɵcf, lowercaseFormatter as ɵcg, maskFormatter as ɵch, multipleFormatter as ɵci, percentFormatter as ɵcj, percentCompleteFormatter as ɵck, percentCompleteBarFormatter as ɵcl, percentSymbolFormatter as ɵcm, progressBarFormatter as ɵcn, translateFormatter as ɵco, translateBooleanFormatter as ɵcp, uppercaseFormatter as ɵcq, yesNoFormatter as ɵcr, avgTotalsFormatter as ɵcs, avgTotalsDollarFormatter as ɵct, avgTotalsPercentageFormatter as ɵcu, maxTotalsFormatter as ɵcv, minTotalsFormatter as ɵcw, sumTotalsFormatter as ɵcx, sumTotalsBoldFormatter as ɵcy, sumTotalsColoredFormatter as ɵcz, SumAggregator as ɵd, sumTotalsDollarFormatter as ɵda, sumTotalsDollarBoldFormatter as ɵdb, sumTotalsDollarColoredFormatter as ɵdc, sumTotalsDollarColoredBoldFormatter as ɵdd, getAssociatedDateSorter as ɵde, numericSorter as ɵdf, objectStringSorter as ɵdg, stringSorter as ɵdh, AutoCompleteEditor as ɵe, CheckboxEditor as ɵf, DateEditor as ɵg, FloatEditor as ɵh, IntegerEditor as ɵi, LongTextEditor as ɵj, MultipleSelectEditor as ɵk, SelectEditor as ɵl, SingleSelectEditor as ɵm, SliderEditor as ɵn, TextEditor as ɵo, executeMappedCondition as ɵp, booleanFilterCondition as ɵq, collectionSearchFilterCondition as ɵr, numberFilterCondition as ɵs, stringFilterCondition as ɵt, testFilterCondition as ɵu, AutoCompleteFilter as ɵv, CompoundDateFilter as ɵw, CompoundInputFilter as ɵx, CompoundInputNumberFilter as ɵy, CompoundInputPasswordFilter as ɵz };
